...
const root2 = admin.database().ref('User2');
...
exports.webhook2 = functions.database.ref('/User2/codeTrigger').onUpdate(async (snap) => {
    var submissionData;   
    var initializePromise = getSubData2();
    initializePromise.then(function(result){
        submissionData = result;
        //console.log('submissionData.language: '+submissionData.language);
        ...
        function (error, response, body) { 
            ...
            if (response) {
                ...
                if (response.statusCode === 200) {
                    console.log('Compiled Output:', response.body.output);
                    root2.child("codeCompiler").set(response.body.output);             
                } else {
                    console.log('error: ', error);
                    reject();
                }
